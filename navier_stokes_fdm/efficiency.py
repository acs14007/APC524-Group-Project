...
This Python script performs fluid flow simulations using the Navier-Stokes equations.
It sets up an environment with defined parameters, boundary conditions, and objects.
The core simulation logic is encapsulated in the `run_simulation` function.

Key Components:
- Imports the necessary modules and classes.
- `run_simulation` initializes the environment, applies boundary conditions, introduces objects,
    and runs the simulation.
- The main section profiles the simulation's performance using cProfile and saves data to
    'simulation_profile.prof'.

This script is part of a broader fluid dynamics simulation project, facilitating simulations, profiling,
    and optimization.
"""
...
def run_simulation():
    ...
    env.boundary_conditions.append(
        LeftSideFixedVelocityBoundaryCondition(u_value=1.0, v_value=0.0)
    )
    env.boundary_conditions.append(
        RightSideFixedVelocityBoundaryCondition(u_value=1.0, v_value=0.0)
    )
    env.boundary_conditions.append(
        TopSideFixedVelocityBoundaryCondition(u_value=0.0, v_value=1.0)
    )
    env.boundary_conditions.append(
        BottomSideFixedVelocityBoundaryCondition(u_value=0.0, v_value=1.0)
    )
